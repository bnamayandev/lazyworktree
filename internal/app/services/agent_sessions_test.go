...
func TestAgentToolAwaitsUser(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{tool: "AskUserQuestion", want: true},
		{tool: "ExitPlanMode", want: true},
		{tool: "  AskUserQuestion  ", want: true},
		{tool: "Bash", want: false},
		{tool: "Edit", want: false},
		{tool: "", want: false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.tool, func(t *testing.T) {
			t.Parallel()
			if got := agentToolAwaitsUser(tt.tool); got != tt.want {
				t.Fatalf("agentToolAwaitsUser(%q) = %v, want %v", tt.tool, got, tt.want)
			}
		})
	}
}

func TestApplyAgentStatusMarksQuestionAsWaiting(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name string
		tool string
		want models.AgentSessionStatus
	}{
		{
			name: "question blocks on the user",
			tool: "AskUserQuestion",
			want: models.AgentSessionStatusWaitingForUser,
		},
		{
			name: "plan approval blocks on the user",
			tool: "ExitPlanMode",
			want: models.AgentSessionStatusWaitingForUser,
		},
		{
			name: "ordinary tool is still work in progress",
			tool: "Bash",
			want: models.AgentSessionStatusExecutingTool,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			applyAgentStatus(session, "assistant", true, tt.tool, false)
			if session.Status != tt.want {
				t.Fatalf("status = %q, want %q", session.Status, tt.want)
			}
			if session.CurrentTool != tt.tool {
				t.Fatalf("CurrentTool = %q, want %q", session.CurrentTool, tt.tool)
			}
		})
	}
}

// A question is itself a tool call, so the recent-tool shortcut in
// resolveAgentActivity must not report it as work in progress.
func TestResolveAgentActivityKeepsRecentQuestionWaiting(t *testing.T) {
	t.Parallel()
	...
	if activity != models.AgentActivityWaiting {
		t.Fatalf("expected a just-asked question to read as waiting, got %q", activity)
	}
}
