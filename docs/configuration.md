...
agent_command: "claude" # Command launched when opening an agent (Enter) for a worktree; runs in a floating zellij pane and continues an existing chat
...
- `agent_command`: command launched when opening an agent (`Enter`) for a worktree (default: `claude`). Under zellij the agent is placed in a large floating pane, leaving the interface live whilst it works; hide the floating layer to background it and press `Enter` again to reveal it. The agent lives as long as its pane. Outside zellij it runs in place, suspending the interface until it exits. When the command is `claude`, LazyWorktree appends `--continue` if the worktree already has a conversation, so you resume rather than start afresh; an explicit `--continue`/`--resume` in your own command is respected as-is.
